...
export const DynamicNavigation = ({
  ...
  return (
    <nav
      ref={navRef}
      className={cn(
        `relative w-full rounded-none backdrop-blur-md border 
    ...
    >
      ...
      <div
        ref={highlightRef}
        className={cn(
          `absolute top-0 left-0 h-full rounded-none transition-all 
      ...
      <ul className="list-none grid grid-cols-4 w-full items-center py-2 relative z-10">
        {links.map((link) => (
          <li
            key={link.id}
            className="rounded-none px-3 flex justify-center"
            id={`nav-item-${link.id}`}
          >
            <a
              href={link.href}
              className={cn(
                ...
                rounded-none font-medium transition-all duration-300 hover:scale-105 
