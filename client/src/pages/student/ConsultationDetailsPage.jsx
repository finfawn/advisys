...
export default function ConsultationDetailsPage() {
  ...
  const handleCancelConsultation = () => {
    ...
    setTimeout(() => {
      console.log('Consultation cancelled:', consultationData.id);
      navigate('/student-dashboard/consultations');
    }, 1000);
  };
  ...
  const handleAddToCalendar = () => {
    ...
    const startDate = new Date(`${consultationData.date}T${consultationData.time.split(' - ')[0]}`);
    const endDate = new Date(`${consultationData.date}T${consultationData.time.split(' - ')[1]}`);
    
    const calendarData = {
      title: `Consultation: ${consultationData.topic}`,
      start: startDate.toISOString(),
      end: endDate.toISOString(),
      location: consultationData.location,
      description: `Consultation with ${consultationData.faculty.name}\n\nStudent Notes: ${consultationData.studentNotes}`
    };

    // Create Google Calendar link
    const googleCalendarUrl = `https://calendar.google.com/calendar/render?action=TEMPLATE&text=${encodeURIComponent(calendarData.title)}&dates=${startDate.toISOString().replace(/[-:]/g, '').split('.')[0]}Z/${endDate.toISOString().replace(/[-:]/g, '').split('.')[0]}Z&location=${encodeURIComponent(calendarData.location)}&details=${encodeURIComponent(calendarData.description)}`;
