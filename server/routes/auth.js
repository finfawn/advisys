...
router.post('/register', async (req, res) => {
  ...
  if (!_isValidName(firstName) || !_isValidName(lastName)) return res.status(400).json({ error: 'Invalid name' });
  if (!_isValidEmail(email) || !_isValidPassword(password)) return res.status(400).json({ error: 'Invalid email or password' });
  ...
  try {
    ...
    if (role === 'student') {
      ...
      await pool.query(
        ...
        [userId, null, deptClean || null, null, 'available']
      );
...
router.post('/firebase-login', async (req, res) => {
  ...
  try {
    ...
    const nameRaw = String(decoded.name || decoded.displayName || '').trim() || email;
    const name = _isValidName(nameRaw) ? nameRaw.replace(/\s+/g,' ').trim() : email;
    ...
    if (!_isValidEmail(email)) return res.status(400).json({ error: 'Invalid email' });
    ...
    if (desiredRole === 'student') {
      ...
      const programClean = _cleanOptionalText(program);
      await pool.query('INSERT INTO student_profiles (user_id, program, year_level) VALUES (?,?,?)', [userId, programClean || null, yr]);
    } else {
      const deptClean = _cleanOptionalText(department);
      await pool.query('INSERT INTO advisor_profiles (user_id, title, department, bio, status) VALUES (?,?,?,?,?)', [userId, null, (deptClean || null), null, 'available']);
      await pool.query('INSERT INTO advisor_modes (advisor_user_id, online_enabled, in_person_enabled) VALUES (?,?,?)', [userId, 1, 1]);
    }
...
router.post('/change-password', authMiddleware, async (req, res) => {
  ...
  if (!_isValidPassword(newPassword)) return res.status(400).json({ error: 'New password is too weak' });
  try {
    ...
    try {
      await pool.query('UPDATE users SET password_hash = ?, must_change_password = 0, password_changed_at = NOW() WHERE id = ?', [new_hash, userId]);
    } catch (_e) {
      try {
        await pool.query('UPDATE users SET password_hash = ?, password_changed_at = NOW() WHERE id = ?', [new_hash, userId]);
      } catch (_e2) {
        try {
          await pool.query('UPDATE users SET password_hash = ?, must_change_password = 0 WHERE id = ?', [new_hash, userId]);
        } catch (_e3) {
...
router.post('/reset-password', async (req, res) => {
  ...
  try {
    ...
    try {
      await pool.query('UPDATE users SET password_hash = ?, password_changed_at = NOW() WHERE id = ?', [new_hash, row.user_id]);
    } catch (_e) {
